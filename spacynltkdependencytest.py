...
nlp=spacy.load("en_core_web_lg") #load the spacy dependency parser
...
def makesometokens(text):
    # Split on strong legal delimiters that indicate a new clause
    rough_sentences = re.split(r'\b(?:comprising|including|wherein|and|;)\b', text)
    sentences = [s.strip() for s in rough_sentences if len(s.strip()) > 5]
    words = word_tokenize(text.lower())
    return sentences, words
...
def setscores(text, freq_table):
    ...
    for sentence in text: #search the input text for each sentence
        doc = nlp(sentence) #use spacy nlp to determine characteristics
        word_count = 0 #tracks the important words' number of occurences in the sentence
        score = 0
        for words in doc:
            word = words.text.lower()
            if word in freq_table:
                base_score = freq_table[word]
                boost = DEP_WEIGHTS.get(words.dep_, 1.0)
